...
this.options.width,this.options.height,0,0,this.options.width,this.options.height);f=d.toDataURL("image/"+this.options.type);b([a,d,c]).remove();b.post(this.options.url,{cropped_file:f}).success(b.proxy(function(a){this.element.trigger("success",a)},this)).error(b.proxy(function(a){this.element.trigger("error",a)},this))},addDragBehaviour:function(){this.area.on("mousedown",b.proxy(function(a){this.area.addClass("b-crop-box__area_down");this.start={left:a.clientX-this.holder.position().left-this.image.position().left,
top:a.clientY-this.holder.position().top-this.image.position().top};b(document).on("mousemove.cropbox",b.proxy(function(a){var b=a.clientX-this.start.left-this.holder.position().left,a=a.clientY-this.start.top-this.holder.position().top;this.image.css({left:b,top:a})},this));b(document).on("mouseup.cropbox",b.proxy(function(){this.area.removeClass("b-crop-box__area_down");b(document).off("mousemove.cropbox");b(document).off("mouseup.cropbox")},this));a.preventDefault()},this))},repositeSelected:function(a){var b=
Math.floor(this.options.width*a/100),a=Math.floor(this.options.height*a/100);this.selected.css({width:b,height:a,left:Math.floor(this.area.width()/2-b/2),top:Math.floor(this.area.height()/2-a/2)});this.masks.eq(0).css({left:this.selected.position().left,top:0,width:b,height:this.selected.position().top});this.masks.eq(1).css({left:this.selected.position().left,bottom:0,width:b,height:this.selected.position().top});this.masks.eq(2).css({left:0,top:0,width:this.selected.position().left,height:"100%"});
this.masks.eq(3).css({right:0,top:0,width:this.area.width()-b-this.selected.position().left,height:"100%"})},onSlide:function(a,b){this.repositeSelected(b)}};b.fn.cropBox=function(){return b(this).data("crop-box",new e(this))};b.fn.cropBox.defaults={width:260,height:350,aspect:0.5,url:"/crop",type:"png"};b(function(){b("body").on("click","[data-crop]",function(a){b(this).cropBox();a.stopPropagation()})})}(window.jQuery);
